	...
	public MensajesDto confirmPurchaseOrder(@RequestBody PurchaseOrderContainer purchaseOrder) {
		...
		try {
			...
			for (AcuseReciboDto response : responses_) {
				if (response.isError()) {
					mensaje.addMensaje(SeveridadType.ERROR, response.getMensaje());
					throw new BadRequestException(mensaje);
				}
			}
			mensaje.addMensaje(SeveridadType.INFO, "Orden confirmada exitosamente");
			return mensaje;
		} catch (Exception e) {
			mensaje.addMensaje(e, purchaseOrder);
			throw new BadRequestException(mensaje);
		}
		// @formatter:on
	}
